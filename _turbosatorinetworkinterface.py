...
This file belongs in C:\Users\<user_name>\.expyriment\extras\expyriment_io_extras
...
class TurbosatoriNetworkInterface(Input, Output):
    ...
    def _send(self, message, *args):
        ...
        data = struct.pack('!q', length + 5 + arg_length) + \
            b"\x00\x00\x00" + chr((length + 1)).encode('utf-8') + message + b"\x00"
        if len(args) > 0:
            for arg in args:
                data += arg
        self._tcp.send(data)
    ...
    def request_data(self, request, *args):
        ...
        elif data.decode('utf-8').startswith("Wrong request!"):
        ...
        # 1. Encode the request to bytes for sending
        request_bytes = request.encode('utf-8')
        self._send(request_bytes, *args)
        ...
        # --- FIX: Use byte string comparison to avoid UnicodeDecodeError ---
        
        # Define the header to look for as a byte string
        ERROR_HEADER = b"Wrong request!"
        
        # Check if the received data starts with the byte representation of the error message
        if data.startswith(ERROR_HEADER):
            # Only decode the error message part, ignoring potential binary data
            error_message = data[len(ERROR_HEADER) + 1:].decode('utf-8', 'ignore')
            raise TurbosatoriNetworkInterface.RequestError(
                "Wrong request! '{0}'!".format(error_message))
        else:
            return data[len(request_bytes)+1:], int((get_time() - start) *1000)
    ...
    def get_values_feedback_folder(self):
        ...
        if folder is None:
            ...
            return folder[4:-1].decode('utf-8'), rt
